...
test.describe('Pagination navigation', () => {
  for (const renderNav of [undefined, 'button']) {
    for (const { initialCurrent, linkToClick, expectedCurrent } of [
      ...
      test(`clicking ${linkToClick}${renderNav ? ` with ${renderNav}` : ''} renders correctly and sets current page to ${expectedCurrent}`, async ({
        page,
        testHarness,
      }) => {
